...
const Beneficiarios: React.FC<BeneficiariosProps> = ({ beneficiarios, setBeneficiarios }) => {
      ...
      <header className="flex flex-col md:flex-row justify-end items-start md:items-center mb-6 gap-4">
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
             <div className="relative">
                <input type="text" placeholder="Buscar no Brincar..." className="pl-10 pr-4 py-2.5 border-2 border-slate-200 rounded-full text-sm w-full sm:w-72 outline-none focus:ring-orange-500 focus:border-orange-500 transition shadow-sm font-medium" />
                ...
                <tr key={b.id} className={b.status === 'busca_ativa' ? 'bg-red-50/30' : 'hover:bg-orange-50/20 transition'}>
                    ...
                    <div className="flex space-x-1.5">
                        {b.frequencia.map((f, idx) => {
                            ...
                            if (f === 'presente') color = 'bg-orange-500';
                            if (f === 'falta') color = 'bg-red-400';
                            return <span key={idx} className={`w-2.5 h-2.5 rounded-full ${color} shadow-sm`} title={f}></span>
                        })}
                    </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
