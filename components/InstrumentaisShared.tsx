...
export const DocumentContainer: React.FC<{ children: React.ReactNode, landscape?: boolean }> = ({ children, landscape }) => (
    <div className={`mx-auto bg-white shadow-xl rounded-sm p-8 md:p-12 font-sans text-black animate-fade-in print:shadow-none print:p-4 border border-gray-100 ${landscape ? 'max-w-[1400px]' : 'max-w-[210mm] min-h-[297mm]'}`}
...
export const SPSFooter: React.FC<{ unit: { name: string; address: string; email: string } }> = ({ unit }) => (
    /* ⚠️ NÃO usar print:fixed — faz o rodapé cobrir o conteúdo nas páginas seguintes.
       O rodapé fica no fluxo normal do documento e aparece ao fim do conteúdo. */
    <footer className="mt-8 pt-3 border-t-[3px] border-black text-sm"
        style={{ breakInside: 'avoid', pageBreakInside: 'avoid' } as React.CSSProperties}>
        <p className="font-bold">{unit.name}</p>
        <p>Endereço: {unit.address}</p>
        <p>E-mail: celuladegestao@sps.ce.gov.br</p>
        <button
            className="print:hidden mt-4 bg-teal-600 text-white px-6 py-3 rounded-xl text-xs font-black uppercase flex items-center gap-2 hover:bg-teal-700 shadow-lg transition transform hover:-translate-y-1"
